...
<table width='90%'>
	<tr>
		...
		<form id='pbox-add-process' action="<?php echo PBox::get_admin_url( 'pb/pb.edit.php', '&amp;action=add_process' );?>" method='post'>
  ...
  <h3 align='center'><?php _e( 'Search', 'pb' );?></h3>
   ...
   <form action='<?php echo Pbox::get_admin_url( 'pb/pb.manage.php', '&amp;action=search_by_id' );?>' method='post'>
...
if ( isset( $_REQUEST['action'] ) && $_REQUEST['action'] == 'search_by_id' ) {
	...
	if ( $_REQUEST['mode'] == 'page_id' ) {
		...
		if( $the_box != -1 && $item_id > 0 ) {
			echo "<p><a href='" . wp_nonce_url( PBox::get_admin_url( "pb/pb.edit.php", "&amp;action=edit_view&amp;box_id=$item_id" ), "pbox-box-edit" ) . "' rel='permalink'>".__( 'Edit PBox', 'pb' )."</a> (ID $item_id)</p>";
		}
		if ( $the_box['pbox_id'] ) {
			//found pbox
			$boxes[0] = $the_box;
...
if( isset( $_REQUEST['action'] ) && $_REQUEST['action'] == 'view_dependencies' ) {
	...
	if ( count( $rows ) == 0 ) {
		...
		if ( strlen( $title ) >= 1 && is_array( $rows ) ) {	
			...
			if( $box_id > 0 ) {
				echo "<p><a href='" . wp_nonce_url( PBox::get_admin_url( "pb/pb.edit.php", "&amp;action=edit_view&amp;box_id=$box_id" ), "pbox-box-edit" ) . "' rel='permalink'>".__( 'Edit PBox', 'pb' )."</a> (ID $box_id)</p>";
			}
...
if( !empty( $boxes ) ) {
	...
	foreach ( (array) $boxes as $box ) {
		...
		<tr id='pbox-<?php echo $box['pbox_id'] ?>' class='<?php echo $alternate ?>'>
			...
			<td width='3%'><a href='<?php echo wp_nonce_url( PBox::get_admin_url( 'pb/pb.edit.php', "&amp;action=edit_view&amp;box_id=".$box['pbox_id'] ), 'pbox-box-edit' ) ?>' class='edit'><?php _e( 'Edit', 'pb' ) ?></a></td>
		<td width="3%">
			<a onclick="jQuery( '#pbox-clone-process' ).attr( 'action', this.href ).submit(); return false;"
			href=" <?php echo PBox::get_admin_url( 'pb/pb.manage.php', "&amp;action=clone_process&amp;box_id={$box['pbox_id']}" ); ?>" rel="permalink" class='edit'><?php _e( 'Clone', 'pb' ); ?></a>
		</td>
		<?php
		if( class_exists( 'XWidgets' ) ) {
			echo "<td width='3%'><a href='" . wp_nonce_url( PBox::get_admin_url( 'pb/pb.manage.php', "&amp;action=view_dependencies&amp;box_id={$box['pbox_id']}" ), 'pbox-boxdependencies-view' ) . "' rel='permalink' class='edit'>" . __( 'Dependencies', 'pb' ) . '</a></td>';
		}
		?>
		<td width='3%'>
			<a onclick="if (confirm('<?php echo esc_js( __( 'Are you sure you want to delete this box?', 'pb' ) ); ?>')) { jQuery('#pbox-delete-process').attr('action', this.href).submit(); }return false;;"
				href="<?php echo PBox::get_admin_url( 'pb/pb.manage.php', "&amp;action=delete_process&amp;box_id={$box['pbox_id']}" ); ?>" rel='permalink' class='delete'><?php _e( 'Delete', 'pb' ); ?></a>
		</td>
		</tr>
		<?php
		if ( $alternate == '' ) {
